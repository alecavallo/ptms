	 ...
	 class TwitterComponent extends Object {
      ...
      $this->connectApp($callback_url, 'authenticate');
		...
		public function homeTimeline() {
			//Return and request
			require_once('vendors/tmhOAuth/tmhOAuth.php');
			//App::import('Vendor', 'tmhOAuth');
			...
			$connection->request('GET', 
		    	$connection->url('1/statuses/home_timeline.json?count=50'), 
		    	array('count' => 50)
		    );
		    $aux = $connection->response['response'];
		    
			//debug($this->apiRequest('get', '/1/statuses/home_timeline.json?count=50', ''));
			//return json_decode($this->apiRequest('get', '/1/statuses/home_timeline.json', $params), true);
		...
		public function showUser($param) {
			...
			if(is_numeric($param)){
				...
				$getParam = 'screen_name';
			}
			//Return and request
			return json_decode($this->apiRequest('get', "/1/users/show.json?{$getParam}={$param}", $body), true);
		}
		//-----
		...
		#Search Methods
	 	#search/search
		/*
		 *Returns a set of tweets whitch matches with the criteria passed as parameter.
		 * The status's author will be returned inline.
		 *
		 * @access public
		 * @return array()
		 * @param int $id The id of the tweet
		 */
		 function search($q = NULL, $lang = 'es', $rpp = NULL, $page = NULL, $since_id = NULL, $geocode = "-37.020098,-56.162109,1116mi") { //se setea por default a argentina, lenguaje español
			...
			return json_decode($this->http->get($url, $args), true);
		}
	 }
?>
